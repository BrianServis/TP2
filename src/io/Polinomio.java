...
public class Polinomio {
	...
	// 		potencia (x, n) = x * potencia (x, n-1)
	 ...
	 public double potenciaParImpar(double x, int n) {
		 if( n < 1 ) return 1;
	...
	public double evaluarPow(double x) { // Adapto la primer funcion de sumas sucesivas
		...
		// Itero por cada potencia termino elevado al cuadrado o mayor en los cuales
		// realizo multiplicaciones sucesivas.
		...
		// Una vez que tengo calculados todos los terminos elevados, le sumo el termino
		// lineal multiplicado por el punto a analizar, mas el termino independiente.
		res += (x * this.coeficientes[this.coeficientes.length - 2] + this.coeficientes[this.coeficientes.length - 1]);
